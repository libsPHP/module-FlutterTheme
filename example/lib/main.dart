...
Future<void> _initializeApp() async {
  try {
    ...
    // Initialize Flutter Magento plugin
    await _initializeMagentoPlugin();
...
Future<void> _initializeMagentoPlugin() async {
  try {
    final magento = FlutterMagento();
    
    // Initialize the plugin with your Magento store URL
    ...
    if (success) {
      if (kDebugMode) {
        print('✅ Flutter Magento plugin initialized successfully');
        print('🔗 Magento Base URL: ${AppConfig.magentoBaseUrl}');
      }
    } else {
      if (kDebugMode) {
        print('❌ Failed to initialize Flutter Magento plugin');
      }
    }
  } catch (e) {
    if (kDebugMode) {
      print('❌ Error initializing Flutter Magento plugin: $e');
    }
  }
}
...
Future<void> _initializeServices() async {
  ...
  if (AppConfig.isFeatureEnabled('enableBiometricAuth')) {
...
class MagentoMarketplaceApp extends ConsumerWidget {
  const MagentoMarketplaceApp({super.key});
